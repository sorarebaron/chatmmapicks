...
def upsert_result(
    ...
    row: dict = {
        ...
        "finish_details": finish_details or None,
...
def get_all_analytics_data() -> dict:
    ...
    # Filter results and picks by known fight_ids in batches of 400 to avoid
    # URL length limits on the IN clause, completely bypassing any server-side
    # global row cap that truncates open-ended queries.
    results: list[dict] = []
    picks: list[dict] = []
